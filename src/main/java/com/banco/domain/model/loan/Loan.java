...
public class Loan {
    ...
    // Regla de Seguridad por Rol (Requisito del documento)
    public void approve(UserRole userRole) {
        ...
        if (this.status != LoanStatus.EN_ESTUDIO) {
            throw new DomainException("El préstamo no se encuentra en fase de estudio.");
        }

        this.status = LoanStatus.APROBADO;
    }

    // Comportamiento: Desembolsar el dinero a una cuenta
    public void disburse(Account targetAccount) {
        if (this.status != LoanStatus.APROBADO) {
            throw new DomainException("No se puede desembolsar un préstamo que no ha sido aprobado.");
        }
